...
const FQA = () => {
  return (
    <section id="faq" className="flex justify-start items-start gap-10 flex-col sm:flex-row py-10">
      <div className="flex items-start justify-center flex-col gap-4 w-full sm:w-1/2">
        ...
        <Link href={"#contact"} className="capitalize border-none  rounded-full  px-5 py-2 sp ">
          CONTACT ME
        </Link>
      </div>
        ...
        <div className="flex flex-col gap-4">
          {Questions.map(({ answer, question, id }) => (
            <label aria-description={question} className="block" htmlFor={id} key={id}>
              ...
              <span className="cursor-pointer w-full text-xl pb-4 text-white flex gap-2 items-center">
                {question}
